...
class SyncYandexController(SyncController):
    ...
    def handle_changes(self, changes: List[Tuple[str, str]]):
        ...
        for file_path, change_type in changes:
            try:
                ...
                if not action:
                    logger.warning(
                        f"[{self.service_name}] Unknown change type: {change_type}"
                    )
                    continue

                logger.info(
                    f"[{self.service_name}] Detected {change_type} file: {file_path}. Processing in cloud."
                )
                ...
                logger.error(
                    f"[{self.service_name}] Error handling {change_type} for {file_path}: {e}"
                )
    ...
    def start(self):
        ...
        """
        logger.info(
            f"[{self.service_name}] Starting folder monitoring and sync process."
        )
    ...
    def stop(self):
        ...
        logger.info(
            f"[{self.service_name}] Stopping folder monitoring and sync process."
        )
    ...
    def _process_response(self, response, action, file_path):
        ...
        if response.status_code == action["success_code"]:
            logger.info(
                f"[{self.service_name}] {action['success_message']} for {file_path}."
            )
        elif response.status_code == action.get("alt_code"):
            logger.info(
                f"[{self.service_name}] {action['alt_message']} for {file_path}."
            )
            operation_link = response.json().get("href")
            if operation_link:
                logger.info(
                    f"[{self.service_name}] Link to check status for {file_path}: {operation_link}."
                )
        else:
            logger.info(
    ...
    def _sync_initial_state(self):
        ...
        """
        logger.info(
            f"[{self.service_name}] Started initial synchronising process"
        )
        ...
        for file_dict in drive_files:
            try:
                ...
                logger.error(
                    f"[{self.service_name}] Error cleaning for {file_dict["name"]} for"
        ...
        for file_path in current_folder_files.keys():
            try:
                ...
                logger.error(
                    f"[{self.service_name}] Error uploading for {file_path}: {e}"
                )
